package dev.jason.app.compose.desktop.messenger.data.api.socket

import dev.jason.app.compose.desktop.messenger.data.api.model.MessageDto
import dev.jason.app.compose.desktop.messenger.data.api.model.toDomain
...
class ApiSocketRepoImpl(private val client: OkHttpClient) : ApiSocketRepository {
    ...
    override suspend fun connect(user: User, roomId: String): Result = suspendCancellableCoroutine { cont ->

        val request = Request.Builder()
            .url("wss://jason-messenger.up.railway.app/chat/$roomId?userId=${user.username}&password=${user.password}")
            .build()

        val messagesRequest = Request.Builder()
            .url("https://jason-messenger.up.railway.app/get-messages/$roomId")
            .build()
        ...
        val listener = object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                socket = webSocket
                println("WebSocket connected to $roomId")
